...
@section('body')
  <nav class="navbar navbar-dark sticky-top flex-md-nowrap p-0 shadow" style="background-color: #3B4EAA">
    <a class="navbar-brand col-md-3 col-lg-2 mr-0 px-3" href="{{ route(Auth::user()->role == 'admin' ? 'admin' : (Auth::user()->role == 'narasumber' ? 'narasumber' : (Auth::user()->role == 'penyelenggara' ? 'penyelenggara' : (Auth::user()->role == 'narasumberprem' ? 'narasumberprem' : 'penyelenggaraprem')))) }}">{{ config('app.name') }}</a>
